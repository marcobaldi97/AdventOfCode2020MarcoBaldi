...
function puzzleDay4a() {
    ...
    myPassportsLanes.forEach(currentItem => {
        ...
        separateElementsBySpace.forEach(currentItem => {
            ...
            if(separateElementsByDoubleStops[0] == 'cid') cidIsHere = true;
        });
        if(passPortsElements-nothingCorrection == 8) correctPassports++;
        if(passPortsElements-nothingCorrection == 7 && cidIsHere == false) correctPassports++;
    });
