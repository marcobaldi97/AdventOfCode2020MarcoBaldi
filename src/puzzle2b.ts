...
function puzzleDay2a(){
    ...
    myConcreteLines.forEach(currentItem => {
        let counterToOut:number = 0;
        let charactersInWord = currentItem.word.split('');
        for (let i:number = 0; i < charactersInWord.length; i++) {
            if(i+1 == currentItem.min && charactersInWord[i] == currentItem.letter) counterToOut++;
            if(i+1 == currentItem.max && charactersInWord[i] == currentItem.letter) counterToOut++;
        };
        if(counterToOut == 1) correctOnes++;
    });
    return correctOnes;
}
